...
void copy_cpio(void *archive) {
	...
	char __attribute__ ((aligned (4096))) path[4096];
	app_ipc_add_stack(5, path, 4096, (1 << 21) | ( 1 << 0));
	...
	while(1) {
        ...
        header = next;
//        cpio_strcpy(buf[i],  current_filename);
		printf("%s, %p (%d)\n", current_filename, cpio_get_file(archive, current_filename, &size), size);
		...
		void *ptr = cpio_get_file(archive, current_filename, &size);
		write(fd, ptr, size);
		close(fd);
    }
}

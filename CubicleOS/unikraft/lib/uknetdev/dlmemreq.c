...
static long ch_size = (10*1024*1024LL);

char* get_memory_netdev(unsigned num_bytes) {
#ifdef DEBUG
    uk_pr_crit("get_mem netdev: %p/%x/%x/%lx\n", ch_start, ch_ctr, ch_size, num_bytes);
#endif
	if(ch_start == NULL) {
//		uk_pr_crit("allocating %x bytes for netdev\n", ch_size);
		ch_start = malloc(ch_size);
		if(ch_start == NULL) {
			uk_pr_crit("cannot allocate memory (%ld) for netdev \n", ch_size);while(1);
		}
		memset(ch_start, 0, ch_size);
		uk_pr_crit("netdev HEAP: %p -- %p (%x)\n", ch_start, ch_start + ch_size, ch_size);
		ch_ctr = 0;
	}

...
int pthread_initcall(void)
{
	uk_pr_crit("Initialize pthread-embedded\n");
	return pthread_init();
}
...
//
	...
	ukboot_ipc_add_heap(ptd, sizeof(pte_thread_data_t), (1 << 0 ) | (1 << 25));
//
...
pte_thread_data_t gptd;
int init = 1;

static void *current_tls(void)
{
//	pte_thread_data_t *ptd = current_ptd();
	pte_thread_data_t *ptd = &gptd;
	if(init) {
		ptd->tls = pteTlsThreadInit();
		init = 0;
	}
